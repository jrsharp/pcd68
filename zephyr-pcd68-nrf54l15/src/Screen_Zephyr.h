...
private:
    ...
    u8 *framebuffer;         // Framebuffer for e-paper output
    u8 *previous_frame;      // Previous frame for dirty detection
    bool dirty;
    bool framebuffer_supported;
    bool is_epaper;          // Always true for nRF54L15 targets
    ...
    // Format conversion for e-paper
    void convertMonoToEpaper(u8 *dest, const u8 *src, int width, int height);
    void scaleFrameToEpaper(u8 *dest, const u8 *src, int dest_w, int dest_h, int src_w, int src_h);
